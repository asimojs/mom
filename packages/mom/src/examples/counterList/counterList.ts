...
export const CounterListStore = storeFactory(CounterListSID, (m, params) => {
    const model = m.makeAutoObservableModel({
        ...
        addCounter() {
            model.counters.push(m.mount({ $store: CounterStore, value: model.$defaultValue }));
        },
        /** Remove a counter */
        removeCounter(c: Counter) {
